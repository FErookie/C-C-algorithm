...
int main() {
    ...
    for (int i = position; i < 9; i++) {
        ...
        queue_list[position].rear->link = queue_list[i].front;
        queue_list[position].rear = temp;
    }
    if (queue_list[9].size != 0) {
        LinkNode <int> * temp = queue_list[9].rear;
        queue_list[position].rear->link = queue_list[9].front;
        queue_list[position].rear = temp;
    }
    while (queue_list[position].front) {
        cout << queue_list[position].front->data;
        queue_list[position].front = queue_list[position].front->link;
    }
    getchar();
    return 0;
}
