...
template <class T>
void BinaryTree<T>::deleteLeaf(BinaryTreeNode<T> *root) {
	if (root == NULL) return;
...
int BinaryTree<T>::getHeight(BinaryTreeNode<T> *root) {
	if (root == nullptr) {
		...
		return (getHeight(root->getLeftChild()) > getHeight(root->getRightChild())) ? getHeight(root->getLeftChild()) + 1 : getHeight(root->getRightChild()) + 1;
	}
}
template <class T>
bool BinaryTree<T>::judgeComplete(BinaryTreeNode<T> *root) {
	int x = 0;
	BinaryTree::findOne(root, x);
	if (x == 0) {
		return true;
	}
	return false;
...
template <class T>
void BinaryTree<T>::findOne(BinaryTreeNode<T> *root, int &x) {
	if (root != nullptr) {
		if ((root->getLeftChild() == nullptr && root->getRightChild()) || (root->getLeftChild() && root->getRightChild() == nullptr)) {
			x++;
		}
		findOne(root->getLeftChild(), x);
		findOne(root->getRightChild(), x);
	}
}
template <class T>
void BinaryTree<T>::findTwo(BinaryTreeNode<T> *root, int &x) {
	if (root != nullptr) {
		if ((root->getLeftChild() && root->getRightChild())) {
			x++;
		}
		findTwo(root->getLeftChild(), x);
		findTwo(root->getRightChild(), x);
	}
}
template <class T>
void BinaryTree<T>::findNone(BinaryTreeNode<T> *root, int &x) {
	if (root != nullptr) {
		if ((root->getLeftChild() == nullptr && root->getRightChild() == nullptr)) {
			x++;
		}
		findNone(root->getLeftChild(), x);
		findNone(root->getRightChild(), x);
	}
}
...
int main() {
	...
	cout << "\n只有一个儿子的节点有" << endl;
	int x = 0;
	tree.findOne(tree.getRoot(), x);
	cout << x << endl;
	cout << "\n有两个儿子的节点有" << endl;
	x = 0;
	tree.findTwo(tree.getRoot(), x);
	cout << x << endl;
	cout << "\n没有儿子的节点有" << endl;
	x = 0;
