...
void myList::modifyItem(int x, int target) {
    ...
    node *selectSortList(node *head) {

        if(head == NULL || head->next == NULL)return head;
        node *pstart = new node();
        pstart->next = head;
        node*sortedTail = pstart;

        while(sortedTail->next != NULL)
        {
            node*minNode = sortedTail->next, *p = sortedTail->next->next;
            while(p != NULL)
            {
...
int main() {
	...
	cout << "******************************" << endl;
	cout << "排序操作" << endl;
    list.head -> next = selectSortList(list.head -> next);
	list.show();
	cout << "******************************" << endl;
	...
	list2.addItem(-1);
	list2.addItem(40);
    list2.head -> next = selectSortList(list2.head -> next);
	list2.show();
	cout << "******************************" << endl;
	cout << "此时平均值" << endl;
